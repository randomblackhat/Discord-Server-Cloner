...
async def guild_info():
    clear()
    logo()
    guild_id = input(f"                {b}[{w}Guild ID{b}]{w} >> ")
    clear()
    logo()
    ...
    if response.status_code == 200:
        ...
        t = current_time()
        print(f"\n                {b}[{w}{t}{b}]{w} {g}[+]{w} Guild Information:")
        print(f"                {b}[{w}{t}{b}]{w} {g}[+]{w} Guild Name: {guild_name}")
        print(f"                {b}[{w}{t}{b}]{w} {g}[+]{w} Guild Icon: {guild_icon_url}")
        print(f"                {b}[{w}{t}{b}]{w} {g}[+]{w} Owner ID: {guild_owner_id}")
        print(f"                {b}[{w}{t}{b}]{w} {g}[+]{w} Region: {guild_region}")
        ...
        print(f"                {b}[{w}{t}{b}]{w} {g}[+]{w} Max Members: {guild_max_members}")
        print(f"                {b}[{w}{t}{b}]{w} {g}[+]{w} Premium Tier: {guild_premium_tier}")
        print(f"                {b}[{w}{t}{b}]{w} {g}[+]{w} Description: {guild_description}")
        ...
        print(f"                {b}[{w}{t}{b}]{w} {r}[-]{w} Failed to fetch guild info: {response.status_code} - {response.text}")
    time.sleep(2)